...
public class RoboCorner extends TeamRobot{
    ...
    public void writeInfo(){
        for(double i=MIN_BULLET_POWER ;i<=MAX_BULLET_POWER;i+=(MAX_BULLET_POWER-MIN_BULLET_POWER )/10){
            System.out.println("La velocitat d'una bala de power "+i+" és de "+getBulletSpeed(i)+" pixels/torn" );
            System.out.println("El dany d'una bala de power "+i+" és de "+getBulletHitBonus(i) );
    ...
    public void goToCorner() throws IOException{
        ...
        sendMessage(nomLider,new Missatge("A quin corner vaig capità?"));
        ...
        if (isTeammate(e.getName())){
           ...
           return;
            ...
            if(kamikaze){
                    ...
                    fire(px, py, e.getHeading(), e.getVelocity());
            }
        }
    }
    ...
    public void fire(double x, double y, double targetHeading, double targetVelocity){
        double dist = distancia(getX(),getY(),x,y);
        if(dist<200){
            double aproxTime=dist/getBulletSpeed(3);
            double dx=x+targetVelocity*Math.sin(Math.toRadians(targetHeading))*aproxTime-getX();
            double dy=y+targetVelocity*Math.cos(Math.toRadians(targetHeading))*aproxTime-getY();
            double theta = Math.toDegrees(Math.atan2(dx, dy));
            turnGunRight(theta-getHeading());
            fire(3);
             System.out.println("En comptes de disparar al punt ("+x+", "+y+") Disparo al punt ("+(dx+getX())+", "+(dy+getY())+")");
        } else if(dist<400){
            double aproxTime=dist/getBulletSpeed(2.5);
            double dx=x+targetVelocity*Math.sin(Math.toRadians(targetHeading))*aproxTime-getX(),dy=y+targetVelocity*Math.cos(Math.toRadians(targetHeading))*aproxTime-getY();
            double theta = Math.toDegrees(Math.atan2(dx, dy));
            turnGunRight(theta-getHeading());
            fire(2.5);
             System.out.println("En comptes de disparar al punt ("+x+", "+y+") Disparo al punt ("+(dx+getX())+", "+(dy+getY())+")");
        } else if(dist<600){
            double aproxTime=dist/getBulletSpeed(2);
            double dx=x+targetVelocity*Math.sin(Math.toRadians(targetHeading))*aproxTime-getX(),dy=y+targetVelocity*Math.cos(Math.toRadians(targetHeading))*aproxTime-getY();
            double theta = Math.toDegrees(Math.atan2(dx, dy));
            turnGunRight(theta-getHeading());
            fire(2); 
            ...
            double aproxTime=dist/getBulletSpeed(1.5);
            double dx=x+targetVelocity*Math.sin(Math.toRadians(targetHeading))*aproxTime-getX(),dy=y+targetVelocity*Math.cos(Math.toRadians(targetHeading))*aproxTime-getY();
            double theta = Math.toDegrees(Math.atan2(dx, dy));
            turnGunRight(theta-getHeading());
            fire(1.5);
            ...
            double aproxTime=dist/getBulletSpeed(1);
            double dx=x+targetVelocity*Math.sin(Math.toRadians(targetHeading))*aproxTime-getX(),dy=y+targetVelocity*Math.cos(Math.toRadians(targetHeading))*aproxTime-getY();
            double theta = Math.toDegrees(Math.atan2(dx, dy));
            turnGunRight(theta-getHeading());
            fire(1);
            ...
            double aproxTime=dist/getBulletSpeed(0.5);
            double dx=x+targetVelocity*Math.sin(Math.toRadians(targetHeading))*aproxTime-getX(),dy=y+targetVelocity*Math.cos(Math.toRadians(targetHeading))*aproxTime-getY();
            double theta = Math.toDegrees(Math.atan2(dx, dy));
            turnGunRight(theta-getHeading());
            fire(0.5);
